...
<html>
       ...
       $(function () {
    	   $("#menu_sort").find('a').toggleClass('on');
      		...
      		$(".goDetail").click(function(){
      			...
      			var mnum = $(this).parents("tr").attr("data-mnum");
      			var img = $(this).parents("tr").find("input[name='pi_secret']").val();
         	...
         	//페이지 버튼 클릭
          	$(".paginate_button a").click(function(e) {
          		e.preventDefault();
          		$("#inq_search").find("input[name='pageNum']").val($(this).attr("href"));
          		goPage();
          	});
	...
	<body>
		...
		<div class="boardContainer container">
			...
			<form id = "inq_search" name="inq_search" class  = "form-inline">
			...
			<div id="sub_menu">
				...
				<div id="menubar">
					<ul id="menu_sort">
						...
						<li><a href="/inq/product-inqList" id="e" class="subMenu_sort" >상품문의</a></li>
					</ul>
				</div>
			</div>
			
			<div id="board_notice">
				<div id="board_title"><h2>상품문의 확인</h2></div>
							...
							<c:choose>
								<c:when test="${not empty inqList}">
									<c:forEach var="inq" items="${inqList }" varStatus="status">
										<tr class= "datas" data-num = "${inq.pi_num }" data-root = "${inq.pi_reproot }" data-mnum = "${inq.m_num }">
											<td class="ac goDetail" >${inq.pi_num }</td>
											<td class="ac goDetail"  style="text-align: left;">
												<c:if test ="${inq.pi_secret eq 'O'}">
													<input type= "hidden" value="${inq.pi_secret }" name = "pi_secret">
													<img class = "secrtimg" src="/resources/images/locked.png" style="width: 30px; height: 30px;">
												</c:if>
												<c:if test ="${inq.pi_secret eq 'X'}">
													<input type= "hidden" value="${inq.pi_secret }" name = "pi_secret">
												</c:if>
												<span>${inq.pi_title }</span>
											</td>
											<td class="ac adad goDetail" style="text-align: center;" >${inq.m_id}</td>
											<td class="ac goDetail" >${inq.pi_regdate }</td>
											<td class="ac goDetail" >${inq.pi_state }</td>
										</tr>						
										<tr data-num = "${inq.pi_num }" data-mnum = "${inq.m_num }" class ="tr-n-content"> 
      										<td colspan="5" class = "n-content no${inq.pi_num }">
      											<div class = "divcontent">
      											<span>${inq.pi_content }</span>
      											</div>	
      										</td>
      									</tr>
									</c:forEach>
