	...
	<head>
		...
		$(function() {
			  $("#menu_sort").find('a').toggleClass('on');
		        ...
		        //이메일 전화번호 자동입력
		    	var email = "${infoqvo.q_email}";
		    	var phone = "${infoqvo.q_phone}";
				
		        $("#m_email").val(email);
		        $("#m_phone").val(phone);
		       
		        var title = "${infoqvo.q_title}";
		        var content = "${infoqvo.q_content}";
		       	content = content.replace(/<br>|<br\/>/g,"\n");
		        
		        $("#in_q_select").val(title);
		        $("#faqcontent").val(content);
		        
		        
		        //셀렉트값 가져와서 변경
		        $(".qna-item > option[value='${infoqvo.q_category}']").attr("selected", "true");
	  			
		        
		        //취소 클릭시 주문조회 값 삭제
		        $("#reset").click(function(){
		        	/* $(this).parents("td").find("#in_o_num").val(); */
		        	$("#in_o_num").val("");
		        });
			...
			<div id="board_notice">
				<div id="board_title"><h2> 1:1문의</h2></div>
      			...
      			<form id = "insert-form" name = "insert-form">
      				<input type = "hidden" id="q_num" name = "q_num" value = "${param.q_num }">
					<table id="notice_table">
						...
						<tbody>
							...
							<tr>
								<th>주문번호</th>
								<td>
									<input type = "text" id = "in_o_num" name = "o_num" value="${infoqvo.o_num}" style="width: 25%" readonly="readonly" class = "qna-input">
									...
									<button type = "button" class = "in-o-btn"  id = "reset">취소</button>
							...
							<tr>
								...
								<td>
									...
									<span style="padding-left:10px">
										...
										<label for = "q_emailsend"  style="font-size:8pt;">답변수신을 문자메시지로 받겠습니다.</label>
									</span>
								</td>
							</tr>
							<tr>
